# 🌐 Moroccan Parliament Legislation Web Viewer

A beautiful, interactive web interface to preview and explore the scraped legislation data from the Moroccan Parliament.
...
```bash
# Start the web server and open viewer automatically
...
Simply open `web_viewer.html` in your web browser (may have CORS restrictions with some browsers).

## 📁 File Structure

```
moroccan-parliament-scraper/
├── web_viewer.html              # Main web interface
├── serve_web_viewer.py          # Auto-server script
├── WEB_VIEWER_README.md         # This file
└── data/
    └── extracted-data-2025.json # Data source (required)
```
...
### 🎨 **Styling**
Edit the `<style>` section in `web_viewer.html` to customize:
...
- Use the provided server script: `python3 serve_web_viewer.py`
